...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        train_lr=1e-4, # 0.0001
        train_num_steps=10000000, # 400000
        ...
        self.step_start_ema = step_start_ema # 2000
        self.save_and_sample_every = save_and_sample_every # 40000

        self.batch_size = train_batch_size # 32
        self.gradient_accumulate_every = gradient_accumulate_every # 2
        self.train_num_steps = train_num_steps # 400000
        ...
        self.window = opt.window # 120

        self.add_language_condition = self.opt.add_language_condition # true

        self.use_random_frame_bps = self.opt.use_random_frame_bps # true

        self.use_object_keypoints = self.opt.use_object_keypoints  # true

        self.add_semantic_contact_labels = self.opt.add_semantic_contact_labels # false

        self.test_unseen_objects = self.opt.test_unseen_objects  # false

        self.save_res_folder = self.opt.save_res_folder

        self.use_object_split = self.opt.use_object_split # false
        ...
        self.input_first_human_pose = self.opt.input_first_human_pose  # true

        self.use_guidance_in_denoising = self.opt.use_guidance_in_denoising  # false

        self.compute_metrics = self.opt.compute_metrics  # false

        self.loss_w_feet = self.opt.loss_w_feet # 1
        self.loss_w_fk = self.opt.loss_w_fk # 0.5
        self.loss_w_obj_pts = self.opt.loss_w_obj_pts # 1

        if self.add_language_condition: # true
            clip_version = 'ViT-B/32'
            self.clip_model = self.load_and_freeze_clip(clip_version)

        self.use_long_planned_path = self.opt.use_long_planned_path # false
        self.test_object_name = self.opt.test_object_name  # ""
        self.test_scene_name = self.opt.test_scene_name  # false
        if self.use_long_planned_path: # false
        ...
        if self.test_unseen_objects: # false
    ...
    def prep_start_end_condition_mask_pos_only(self, data, actual_seq_len):
        ...
        mask = torch.ones_like(data[:, :, :3]).to(data.device) # BS X T X 3 [32, 120, 3]
    ...
    def get_gt_res_npz(self):
        ...
        for s_idx, val_data_dict in enumerate(test_loader):
            ...
            sample_idx = 0
            vis_tag = "_sidx_"+str(s_idx)+"_sample_cnt_"+str(sample_idx)
            ...
            self.gen_vis_res_generic(for_vis_gt_data, val_data_dict, 10, vis_gt=True, \
            ...
            for tmp_bs_idx in range(tmp_bs):
                ...
                gt_seq_res_npz_path = self.save_res_folder + "_gt"
                gt_seq_res_npz_path = os.path.join(gt_seq_res_npz_path, "res_npz_files")
                gt_seq_res_npz_path = os.path.join(gt_seq_res_npz_path, \
                                                        tmp_seq_name+".npz")
                gt_global_jpos = gt_human_jnts_list[0].detach().cpu().numpy()
                np.savez(gt_seq_res_npz_path, seq_name = tmp_seq_name, global_jpos = gt_global_jpos)
    ...
    def cond_sample_res(self):
        ...
        total_params = sum(p.numel() for p in self.ema.ema_model.parameters()) / 1e6
        print(f"\n[Efficiency Metrics] Model Parameters: {total_params:.2f} M")
        if self.test_on_train:
            test_loader = torch.utils.data.DataLoader(
                self.ds, batch_size=1, shuffle=False,
                num_workers=1, pin_memory=True, drop_last=False)
        ...
        for s_idx, val_data_dict in enumerate(test_loader):
            ...
            max_vram_allocated = max(max_vram_allocated, peak_vram)
            num_evaluated_seqs += 1
            ...
            for tmp_s_idx in range(num_samples_per_seq):
                ...
                gt_floor_height, pred_floor_height, gt_d_hand, pred_d_hand = \
            ...
            if num_evaluated_seqs > 0:
                avg_inf_time = total_inference_time / num_evaluated_seqs
                print(f"\n================ Deployment Costs Summary ================")
                print(f"Total Parameters: {total_params:.2f} M")
                print(f"Peak VRAM Usage:  {max_vram_allocated:.3f} GB")
                print(f"Avg Inference Time: {avg_inf_time:.4f} s/seq")
                print(f"==========================================================\n")

            if len(self.all_pred_d_hand_list) > 0:

                valid_pred_d_hand = [val for val in self.all_pred_d_hand_list if val > 0]
                valid_gt_d_hand = [val for val in self.all_gt_d_hand_list if val > 0]
    ...
    def cond_sample_res_key_finding(self):
        ...
        for s_idx, val_data_dict in enumerate(test_loader):
            ...
            # obj_bps_data = val_data_dict['input_obj_bps'].to(device)
            with torch.no_grad():



                if hasattr(self.ema.ema_model, 'bps_encoder'):
                    bps_key = self.ema.ema_model.bps_encoder(obj_bps_data) # [BS, 1, 256]
                elif hasattr(self.ema.ema_model.denoise_fn, 'bps_encoder'):
                    bps_key = self.ema.ema_model.denoise_fn.bps_encoder(obj_bps_data)
                else:

                    bps_key = obj_bps_data[:, :, :256]

            text_data = val_data_dict['text']
            with torch.no_grad():



                # 1. Tokenize / Encode
                text_tokens = self.encode_text(text_data)


                if isinstance(text_tokens, torch.Tensor):
                    text_tokens = text_tokens.to(device)

                if hasattr(self.ema.ema_model, 'clip_encoder'):
                    text_key = self.ema.ema_model.clip_encoder(text_tokens) # [BS, 1, 512]
                else:
                    text_key = text_tokens
            ...
            def get_hook(name):
                def hook(module, input, output):
                    ...
                    if isinstance(output, tuple):
                        ...
                        feat = output
                    hidden_states[name] = feat.detach()
                return hook



            transformer = self.ema.ema_model.denoise_fn.motion_transformer
            ...
            # ==========================================
            results = {'layers': [], 'text_corr': [], 'bps_corr': []}


            layer_names = sorted(hidden_states.keys(), key=lambda x: int(x.split('_')[1]))

            for name in layer_names:

                feat = hidden_states[name].mean(dim=1) # [BS, 512]


                # text_key: [BS, 1, 512] -> squeeze -> [BS, 512]
                sim_text = F.cosine_similarity(feat, text_key.squeeze(1), dim=-1).mean().item()


                # bps_key: [BS, 1, 256] -> [BS, 256]





                target_bps = bps_key.squeeze(1) # [BS, 256]
                target_feat = feat[:, :target_bps.shape[-1]]

                sim_bps = F.cosine_similarity(target_feat, target_bps, dim=-1).mean().item()

                results['layers'].append(name)
                results['text_corr'].append(sim_text)
                results['bps_corr'].append(sim_bps)


            for h in hooks: h.remove()


            plot_geometric_dilution(results, save_path="./ours_geometric_dilution_evidence_"+str(s_idx)+".png")
...
def run_train(opt, device):
    ...
    # Define model
    repr_dim = 3 + 9 # Object relative translation (3) and relative rotation matrix (9)   12

    repr_dim += 24 * 3 + 22 * 6 # Global human joint positions and rotation 6D representation 72 + 132

    if opt.use_object_keypoints: # true
        repr_dim += 4  # 220

    loss_type = "l1"
    opt.loss_type = loss_type
    # save_opt_to_yaml(opt, "./configs/chois_omomo.yaml")
    # d_model = 512(default)
    # n_dec_layers = 4, n_head = 4, d_k = 256, d_v = 256, window = 120, repr_dim = 220 input_first_human_pose = true, use_object_keypoints = true
...
def parse_opt():
    ...
    parser.add_argument("--input_first_human_pose", action="store_true") # true
