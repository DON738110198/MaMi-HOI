...
class CanoObjectTrajDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        window_obj_rot_mat = torch.from_numpy(self.window_data_dict[index]['obj_rot_mat']).float() # [85, 3, 3]

        # Prepare relative rotation
        if self.use_random_frame_bps:
            reference_obj_rot_mat = window_obj_rot_mat[random_sampled_t_idx:random_sampled_t_idx+1] # [1, 3, 3]
        ...
        global_joint_rot = data_input[:, 2*num_joints*3:]

        new_data_input = torch.cat((normalized_jpos.reshape(-1, num_joints*3), global_joint_rot), dim=1) # [85, 204]
        ori_data_input = torch.cat((data_input[:, :num_joints*3], global_joint_rot), dim=1) # [85, 204]
        ...
        if self.use_object_keypoints:
            ...
            sampled_nn_pts_on_mesh = nn_pts_on_mesh[sampled_vidxs] # K X 3  [100, 3]
        ...
        if actual_steps < self.window:
            paded_new_data_input = torch.cat((new_data_input, torch.zeros(self.window-actual_steps, new_data_input.shape[-1])), dim=0) # [120, 204]
            ...
            paded_obj_com_pos = torch.cat((torch.from_numpy(self.window_data_dict[index]['window_obj_com_pos']).float(), \
                torch.zeros(self.window-actual_steps, 3)), dim=0) # [120, 3]
            ...
            if self.use_random_frame_bps:
                paded_rel_obj_rot_mat = torch.cat((window_rel_obj_rot_mat, \
                    torch.zeros(self.window-actual_steps, 3, 3)), dim=0) # [120, 3, 3]
