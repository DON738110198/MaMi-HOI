...
class TransformerDiffusionModel(nn.Module):
    def __init__(
        ...
        self.control_transformer = copy.deepcopy(self.motion_transformer)



        self.hint_block = nn.Sequential(
            nn.Linear(self.d_feats, self.d_model),
            nn.ReLU(),
            nn.Linear(self.d_model, self.d_model)
        )

        self.zero_convs = nn.ModuleList([
            zero_module(nn.Linear(self.d_model, self.d_model))
        ...
        self.gaa_block = GeometryAwareProximityAdapter(self.d_model, self.n_head, self.d_k, self.d_v)
    ...
    def forward(self, src, noise_t, condition = None, language_embedding=None, padding_mask=None):
        ...
        if condition is not None:
            x_pose_cond = condition[:, :, -self.d_feats:] # [BS, T, 220]

            # [BS, T, 220] -> [BS, T, 512] -> [BS, 512, T]
            hint_emb = self.hint_block(x_pose_cond).transpose(1, 2)

            ctrl_features, _ = self.control_transformer(
                data_input,
                padding_mask,
                pos_vec,
                obj_embedding=noise_t_embed + hint_emb.transpose(1, 2)[:, :1, :]

            )

            control_signals = []

            if isinstance(ctrl_features, list):
                for i, layer_feat in enumerate(ctrl_features):
                    # layer_feat: [BS, T, D_model]
                    control_signals.append(self.zero_convs[i](layer_feat))
            else:

                control_signals = [self.zero_convs[-1](ctrl_features)] * self.n_dec_layers
        ...
        motion_feats = feat_pred[:, 1:] # [BS, T, 512]
        if condition is not None:
            ...
            feat_final = motion_feats
        output = self.linear_out(feat_final) # BS X T X D
...
class ObjectCondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        p2_loss_weight_k = 1,
        input_first_human_pose=False,  # true
        use_object_keypoints=False, # True
    ):
        super().__init__()
        ...
        self.input_first_human_pose = input_first_human_pose  # true
        ...
        self.seq_len = max_timesteps - 1  # 120
        self.out_dim = out_dim  # 220

        if beta_schedule == 'linear':
            betas = linear_beta_schedule(timesteps)
        elif beta_schedule == 'cosine': # cosine
    ...
    def sample(self, x_start, ori_x_cond, cond_mask=None, padding_mask=None, \
        ...
        if ori_x_cond is not None:
            ...
            bps_time_ms = start_event.elapsed_time(end_event)

            print(f"[Rebuttal Profiler] BPS Encoder Time (BS={1}): {bps_time_ms:.4f} ms")
            # ==========================================================

            # ==========================================================
            x_cond = x_cond.repeat(1, self.seq_len, 1) # BS X T X (3+256)  [1, 120, 256]
        else:
            x_cond = None
    ...
    def p_losses(self, x_start, x_cond, t, language_embedding=None, noise=None, \
        ...
        noise = default(noise, lambda: torch.randn_like(x_start)) # [32, 120, 220]
        ...
        if self.objective == 'pred_noise':
            target = noise
        elif self.objective == 'pred_x0': # here
            target = x_start
        else:
            raise ValueError(f'unknown objective {self.objective}')
        ...
        loss_reshaped = loss.reshape(x_start.shape[0], self.seq_len, -1)  # [32, 120, 220]
        ...
        if loss_reshaped.shape[-1] == 12: # objetc motion only
            ...
            loss_human = loss_reshaped[:, :, 12:] # [32, 120, 208]
        ...
        if self.use_object_keypoints:
            ...
            gt_global_jpos = target[:, :, 12:12+24*3].reshape(bs, num_steps, 24, 3) # [32, 120, 24, 3]
            gt_global_jpos = ds.de_normalize_jpos_min_max(gt_global_jpos) # BS X T X 24 X 3  [32, 120, 24, 3]
            ...
            human_jnts = human_jnts.reshape(bs, num_steps, 24, 3) # BS X T X 24 X 3  [32, 120, 24, 3]
            ...
            rest_pose_obj_kpts = data_dict['rest_pose_obj_pts'].to(model_out.device) # BS X K X 3  [32, 100, 3]
            gt_seq_obj_kpts = data_dict['ori_obj_keypoints'].to(model_out.device) # BS X T X K X 3  [32, 120, 100, 3]
    ...
    def forward(self, x_start, ori_x_cond, cond_mask=None, padding_mask=None, \
        ...
        if ori_x_cond is not None:
            ...
            print(f"[Rebuttal Profiler] BPS Encoder Time (BS={bs}): {bps_time_ms:.4f} ms")
