...
class npy2obj:
    def __init__(self, motion, npy_path, sample_idx, rep_idx, device=0, cuda=True, if_color=False):
        ...
        if self.nfeats == 3:
            ...
            motion_tensor, opt_dict = self.j2s.joint2smpl(self.motions['motion'][self.absl_idx].permute(2, 0, 1))  # [nframes, njoints, 3] [1, 25, 6, 196]
            self._motions = motion_tensor.cpu().numpy()
        ...
        self.vertices = self.rot2xyz(torch.tensor(self._motions), mask=None,
                                     pose_rep='rot6d', translation=True, glob=True,
                                     jointstype='vertices',
                                     # jointstype='smpl',  # for joint locations
                                     vertstrans=False)
        self.root_loc = self._motions[:, -1, :3, :].reshape(1, 1, 3, -1)
    ...
    def save_npy(self, save_path):
        ...
        data_dict = {
            'motion': self._motions[0, :, :, :self.real_num_frames],
            'thetas': self._motions[0, :-1, :, :self.real_num_frames],
            'root_translation': self._motions[0, -1, :3, :self.real_num_frames],
            'faces': self.faces,
...
class npy2obj_object:
    def __init__(self, motion, npy_path, obj_path, sample_idx, rep_idx, device=0, cuda=True, if_color=False):
        self.npy_path = npy_path
        # self.motions = np.load(self.npy_path, allow_pickle=True)
        self.motions = motion
        if self.npy_path.endswith('.npz'):
    ...
    def pose2mesh(self, motion_obj, obj_path, obj_name):
        ...
        for b in range(self.bs):
            ...
            vertices -= center # [498, 3]
            # transform
            angle, trans = motion_obj[b, 0, :3], motion_obj[b, 0, 3:]
            rot = Rotation.from_rotvec(angle.transpose(1, 0)).as_matrix() # (196, 3, 3)
            vertices = torch.from_numpy(np.matmul(vertices[np.newaxis], rot.transpose(0, 2, 1)[:, np.newaxis])[:, 0]) + trans.transpose(1, 0)[:, np.newaxis] # [196, 498, 3]
            vertices = vertices.permute(1, 2, 0) # 498, 3, 196
