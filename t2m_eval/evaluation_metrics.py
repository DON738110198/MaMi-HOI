...
def compute_metrics(ori_verts_gt, ori_verts_pred, ori_jpos_gt, ori_jpos_pred, human_faces, \
    ...
    pred_d_hand_sum = 0
    gt_d_hand_sum = 0
    valid_contact_frames = 0

    for idx in range(num_steps):

        if gt_lhand_contact[idx] or gt_rhand_contact[idx]:
            valid_contact_frames += 1


            # ori_verts_pred[idx]: Nv x 3, pred_obj_verts[idx]: No x 3
            pred_dist_mat = torch.cdist(ori_verts_pred[idx], pred_obj_verts[idx], p=2)
            gt_dist_mat = torch.cdist(ori_verts_gt[idx], gt_obj_verts[idx], p=2)


            pred_d_hand_sum += pred_dist_mat.min()
            gt_d_hand_sum += gt_dist_mat.min()


    if valid_contact_frames > 0:
        pred_d_hand = (pred_d_hand_sum / valid_contact_frames).item() * 1000.0
        gt_d_hand = (gt_d_hand_sum / valid_contact_frames).item() * 1000.0
    else:
        pred_d_hand = 0.0
        gt_d_hand = 0.0
    # ==========================================================
    ...
    obj_rot_dist, obj_com_pos_err, start_obj_com_pos_err, end_obj_com_pos_err, waypoints_xy_pos_err, gt_floor_height, floor_height, gt_d_hand, pred_d_hand
